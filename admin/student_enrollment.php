...
<div class="col-sm-9">
<h1 class="display-6 p-3 text-center">Student Enrollement<p class="fs-6">(Batch/Course/Assign to Students)</p></h1>
<table id="example" class="display">
    ...
    <tbody>
        ...
        <?php  $query=" SELECT * FROM users u JOIN  user_role 
                            ...
                            AND ur.`role_id`= 1 AND ur.`status_id` = 1 ) 
...
<?php 
 if(isset($_REQUEST['enroll_batch_course'])){
    ...
    $record = mysqli_query($connection, $query);
        if($record) {  header("location:student_enrollment.php"); }  
    }
?>
</div>
