...
<!-- data Table Record -->
    <table id="example" class="display" style="width:100%">
        ...
        <tbody>
            ...
            <?php $query = "SELECT * FROM users, user_role WHERE user_role.`user_id`=users.`user_id` AND user_role.`role_id`=3 AND user_role.`status_id`=1";
            ...
            <?php while($row = mysqli_fetch_assoc($result)){ ?>
                ...
                <td><?php echo $row['home_town']; ?></td>    
                    <?php $query = "SELECT role_id FROM user_role WHERE role_id='3' AND user_id ='".$row['user_id']."' ";
